<template>
  <div class="dark:bg-gray-900 bg-white dark:border-none border border-purple-600 rounded-xl p-6 flex flex-col">
    ...
    <div class="text-3xl font-bold mb-6" v-if="price !== 0">${{ price }}<span v-if="priceSuffix !== ''" class="text-sm text-gray-400">{{ priceSuffix || $t('pricing.perMonth') }}</span></div>
    ...
    <button type="button" v-if="price !== 0" class="w-full mt-6 bg-purple-700 hover:bg-purple-600 text-white py-3 rounded-lg cursor-pointer" @click="subscribe">{{ $t(ctaKey || 'pricing.subscribe') }}</button>
